...
class GaleriController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $q = $request->get('q');
            $data['galeri'] = $this->galeriRepository->galeriAdmin()
                ->when(
                    $q ?? false,
                    fn ($query) =>
            ...
            return view('admin.galeri.galeri.fetch', compact('data'))->render();
        }

        $data['galeri'] = $this->galeriRepository->galeriAdmin()
            ->paginate($this->perPage);

        return view('admin.galeri.galeri.index', compact('data'));
    }

    public function show(Galeri $galeri)
    {
        $path = asset('storage/galeri/' . $galeri->filename);
        $url = route('admin.galeri.delete', $galeri->id);
        $output = '
    ...
    {
        $output = '
        <form action="' . route('admin.galeri.store') . '" enctype="multipart/form-data">
                <div class="modal-header">
    ...
    {
        $req = $this->galeriRepository->storeGaleri($request);
    ...
    {
        $req = $this->galeriRepository->deleteGaleri($galeri);
        if ($req['status'] == 'success') {
            return response()->json([
                'data' => $req
            ], $req['status_code']);
